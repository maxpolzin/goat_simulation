...
%         end
    ...
    if pic_num == 1
        imwrite(Amap, map, 'Eye_inplane_V2.gif', 'gif', 'Loopcount', Inf, 'DelayTime', 0.1);
        % 将第一张图片写入‘sin.gif’文件中，并且将GIF播放次数设置成无穷，即保存的GIF图会一直动下去
    else
        imwrite(Amap, map,'Eye_inplane_V2.gif','gif','WriteMode','append','DelayTime',0.1);
        % 依次将其他的图片写入到GIF文件当中
    end
    pic_num = pic_num + 1;
    pause(0.1)
...
save('Eye_Inplane_V2.mat',"Q_sec_M1","U_M1_rd1","U_M1_rd2")
%% caculate bisymmetric configurations in the range inside the boundary
load('Eye_Inplane_V2.mat',"Q_sec_M1")
...
%             Damp=0.01
        ...
        if ismember(nn,1:20)
            ...
            Q_sec(3,N_e_1+2)=Q_sec(3,N_e_1+2)-4;%0.4*(10-0.5*nn);% add the z axis distorion
        else
            Q_sec(3,1)=Q_sec(3,1);
        ...
        save('Eye_Biaxis_Outplane_V2.mat',"Q_sec_M2",'U_M1_rd1','U_M1_rd2')
