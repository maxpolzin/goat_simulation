function Rod_ploting3(q_node,Duplication_M,L_co,Option)
%plot the rod
...
for jj=1:size(Duplication_M,1)
    for ii=1:size(q_node,3)
        ...
        if Option>0
            ...
            surf(surf1(:,:,1),surf1(:,:,2),surf1(:,:,3),C_surf,FaceColor="interp",EdgeColor='none')

        end

        for kk=[1 size(q_node,3)]
            t = linspace(0, 2*pi,36);
            r = 1;
            x = L_co*cos(t);
            y = L_co*sin(t);
            if kk==1
                P(1:3,1)=q_node(1:3,1,kk);
                RM=reshape(q_node(4:12,1,kk),3,3);
            else
                P(1:3,1)=q_node(1:3,end,kk);
                RM=reshape(q_node(4:12,end,kk),3,3);
            end
            TM=[RM,P;[0 0 0 1]];
