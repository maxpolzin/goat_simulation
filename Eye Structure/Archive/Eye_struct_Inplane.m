...
% the middle distance 
L_mid=R0_ring*0.3;
L_co=R0_ring/25;%the length of the coordinates

L_sec_0=1*R0_ring*pi/4;% set as the 1/4circle 
R_rod=0.5;%radius of the rod
E=2e9;%elastic modulus 10kPa
...
flag1=1;
flag3=0;
while (kk==0||flag1>0.0001*R0_ring||flag2>0.10)&&kk<2000
    kk=kk+1
    ...
    dQ_sec_2=-pinv(dFq_dq_sec_2)*Proj_M_rd1'*Fq_sec_M(:,mm);
    dQ_sec=Proj_M_rd1*dQ_sec_2;
    dQ_sec=reshape(dQ_sec,12,[]);
    Q_sec_rd1=Q_sec_rd1+dQ_sec;
...
N_x=25;
X0=Q_sec_rd1(1,1);
disp_x=linspace(0,X0,N_x+1);% sampling displacement x
L_co=2;%the length of the coordinates
C_coord=[.0 .45 .74; .74 .45 .0];%the color of the coordinates
...
%         AA(:,17)
        ...
        dQ_sec=reshape(dQ_sec,12,[]);
        sqrt(sum((Q_sec(1:3,(N_e_0)+1)-Q_sec(1:3,(N_e_1+1+N_e_0)+1)).^2))
