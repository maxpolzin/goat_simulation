...
Damp=0.05;%reduce the iteration speed
mm=1;
kk=0;
flag1=1;
flag3=0;
...
% for nn=1:(size(Q_sec_rd1,2)-1)
%     q_node=Curve_interp(Q_sec_rd1(:,nn),Q_sec_rd1(:,nn+1),L_e_0,N_node);
Rod_ploting(Q_sec_rd1,Duplication_M,L_co,C_coord)
%     q_node=Curve_interp(Q_sec_rd2(:,nn),Q_sec_rd2(:,nn+1),L_e_0,N_node);
Rod_ploting(Q_sec_rd2,Duplication_M,L_co,C_coord)
% end
hold off
box on
grid on
...
%mid point
%couple  X Y Z disp with rd12 middle point XYZ and local frame
...
Proj_M(12*(N_e_0)+(1:3),12*(N_e_0)+(10:12)-12);
%%rod 2
Proj_M_rd2=eye(12*(N_e_1+1),12*(N_e_1+1));%dedemesionalize the variables
% start point
D_fix_rd2=0*Q_sec_rd1;%fixed dimensions/varibales
%contrain  X Y Z disp with start point of Rod1
D_fix_rd2(1,1)=1;%constrain Y disp
D_fix_rd2(2,1)=1;%constrain Y disp
D_fix_rd2(3,1)=1;%constrain  Z disp

Proj_M(12*(N_e_1+1)+1,1)=1;
Proj_M(12*(N_e_1+1)+2,2)=1;
Proj_M(12*(N_e_1+1)+3,3)=1;
...
%end point
%contrain  X Y Z disp with end point of Rod1
D_fix_rd2(1,end)=1;%constrain X disp
D_fix_rd2(2,end)=1;%constrain Y disp
D_fix_rd2(3,end)=1;%constrain Z disp
Proj_M(12*(2*N_e_1+1)+1,12*N_e_1+1)=1;
Proj_M(12*(2*N_e_1+1)+2,12*N_e_1+2)=1;
Proj_M(12*(2*N_e_1+1)+3,12*N_e_1+3)=1;

% Fix local frame
D_fix_rd2(4:12,end)=1;%fix the rotation

%middle point
%contrain  X Y Z disp with rd12 middle point  XYZ and local
%frame
...
%contrain local frame with rd12 middle point frame
D_fix_rd2(4:12,N_e_0+1)=1;%constrain local frame
Con_M_3=eye(9);
Proj_M(12*(N_e_1+1+N_e_0)+(4:12),12*(N_e_0)+(4:12))=Con_M_3;
...
for mm=1:N_x+1
    ...
    while (kk<=100||flag1>0.000001*R0_ring||flag2>0.10)&&kk<2000
        ...
        [dFq_dq_sec_rd2,Fq_sec_M1_rd2(:,mm),q_node_rd2,U_M2_rd2(mm)] =Jocob_rod_sec(Q_sec_rd2,N_e_1,L_e_1,N_node,Par_E,A);
        ...
        %         Fq_sec_M1(12*(N_e_0)+(1:12),mm)
        %         Fq_sec_M1(12*(N_e_0+N_e_1+1)+(1:12),mm)
        %         Q_sec_rd1(:,N_e_0+1)
        %         Q_sec_rd2(:,N_e_0+1)
        %         Fq_sec_M1_rd1(12*(N_e_0)+(1:12),mm);
        %         Fq_sec_M1_rd2(12*(N_e_0)+(1:12),mm);
        %         reshape(Fq_sec_M1_rd1(12*(N_e_0)+(1:12),mm),3,[]);
        %         BB=reshape(Fq_sec_M1_rd1(:,mm),12,[]);
        %         BB(:,17)
        %         AA(:,17)
        %         Proj_M(12*(N_e_0)+(1:12),12*(N_e_0)+(1:12)-12)
        ...
        %         [dQ_sec_2(12*(N_e_0)+(1:12)-12),  dQ_sec(:,(N_e_0)+1), dQ_sec(:,(N_e_1+1+N_e_0)+1)]
    ...
    clf
    hold on
    Rod_ploting(q_node_rd1,Duplication_M,L_co,C_coord)
    Rod_ploting(q_node_rd2,Duplication_M,L_co,C_coord)
    %%plotting wheels
    P_wheel=[];
...
save('Eye_BiSym_Inplane.mat',"Q_sec_M1")
%% caculate bisymmetric configurations in the range inside the boundary
load('Eye_BiSym_Inplane.mat',"Q_sec_M1")
load('Eye_BiSym_Outplane2.mat',"Q_sec_M2")
A=1e9;
...
% start point
D_fix_rd1=0*Q_sec_rd1;%fixed dimensions/varibales
D_fix_rd1(1,1)=1;%fix  X disp
D_fix_rd1(2,1)=1;%fix the Y disp
D_fix_rd1(3,1)=0;%free the  Z disp
% free the x aixs of the local frame
D_fix_rd1(4:6,1)=1;%fix the X axis
D_fix_rd1(7:12,1)=0;%free the y Z axis
...
D_fix_rd1(7:12,end)=0;%free the Y Z axis
...
%contrain  local frame with start point of Rod1
D_fix_rd2(4:12,1)=1;%Constrain the rotation
Con_M_1=eye(9);
Proj_M(12*(N_e_1+1)+(4:12),4:12)=Con_M_1;
...
% contrain  local frame with end point of Rod1
D_fix_rd2(4:12,end)=1;%fix the rotation
Con_M_1=eye(9);
Proj_M(12*(2*N_e_1+1)+(4:12),4:12)=Con_M_1;

%middle point
%contrain  X Y Z disp with wheel frame point (the middle point of rd1 rd2 middle points)
D_fix_rd2(1,N_e_0+1)=1;%constrain X disp
D_fix_rd2(2,N_e_0+1)=1;%constrain Y disp
D_fix_rd2(3,N_e_0+1)=1;%constrain Z disp
...
%contrain local frame with wheel frame point 
D_fix_rd2(4:12,N_e_0+1)=1;%constrain local frame
Con_M_3=eye(9);
Proj_M(12*(N_e_1+1+N_e_0)+(4:12),12*(N_e_0)+(4:12))=Con_M_3;
D_fix=[D_fix_rd1,D_fix_rd2];
...
%         end
        ...
        if ismember(nn,1:4)
            Q_sec(3,1)=Q_sec(3,1)+0.5*(10-0.5*nn);% add the z axis distorion
            Q_sec(3,N_e_1+2)=Q_sec(3,N_e_1+2)+0.5*(10-0.5*nn);% add the z axis distorion
        else
            Q_sec(3,1)=Q_sec(3,1);
        ...
        kk=0;
        flag1=1;
        while (kk<=100||flag1>0.000001*R0_ring||flag2>0.10)&&(kk<5/Damp||kk<2000)
            kk=kk+1;
            [dFq_dq_sec_rd1,Fq_sec_M1_rd1(:,mm),q_node_rd1,U_M1_rd1(mm)] =Jocob_rod_sec(Q_sec_rd1,N_e_1,L_e_1,N_node,Par_E,A);
            [dFq_dq_sec_rd2,Fq_sec_M1_rd2(:,mm),q_node_rd2,U_M2_rd2(mm)] =Jocob_rod_sec(Q_sec_rd2,N_e_1,L_e_1,N_node,Par_E,A);
            dFq_dq_sec=[dFq_dq_sec_rd1,0*dFq_dq_sec_rd1;0*dFq_dq_sec_rd1,dFq_dq_sec_rd2];
            ...
            dQ_sec=Proj_M*dQ_sec_1;
            dQ_sec=reshape(dQ_sec,12,[]);
%             sqrt(sum((Q_sec(1:3,(N_e_0)+1)-Q_sec(1:3,(N_e_1+1+N_e_0)+1)).^2))%the distance between two mid points of rod1 and rod2
            Q_sec=Q_sec+Damp*dQ_sec;            
            Q_sec_rd1=Q_sec(:,1:(N_e_1+1));
            Q_sec_rd2=Q_sec(:,(N_e_1+2):(2*(N_e_1+1)));
%             P_wheel(:,1)=Q_sec_rd1(1:3,N_e_0+1);
%             P_wheel(:,2)=Q_sec_rd2(1:3,N_e_0+1);
%             P_wheel(:,3)=mean(P_wheel(1:3,:),2);
%             Q_sec(1:3,N_e_0+1)=P_wheel(:,3)+(P_wheel(:,1)-P_wheel(:,2))...
%             /norm(P_wheel(:,1)-P_wheel(:,2))*L_mid/2;
%             Q_sec(1:3,N_e_1+1+N_e_0+1)=P_wheel(:,3)-(P_wheel(:,1)-P_wheel(:,2))...
%             /norm(P_wheel(:,1)-P_wheel(:,2))*L_mid/2;
        ...
        save('Eye_BiSym_Outplane2.mat',"Q_sec_M2")
...
% save('Eye_BiSym_Outplane.mat',"Q_sec_M2",'-append')
%%
load('Eye_BiSym_Outplane1.mat',"Q_sec_M2")
pic_num=1
for nn=1:length(disp_y)
...
%                 set(h, 'ZDir', 'reverse');
                ...
                [Amap, map] = rgb2ind(Im, 256); % 将RGB图片转化为索引图片
                if pic_num == 1
                    imwrite(Amap, map, 'Eye_Outplane1.gif', 'gif', 'Loopcount', Inf, 'DelayTime', 0.1);
                    % 将第一张图片写入‘sin.gif’文件中，并且将GIF播放次数设置成无穷，即保存的GIF图会一直动下去
                else
                    imwrite(Amap, map,'Eye_Outplane1.gif','gif','WriteMode','append','DelayTime',0.1);
                    % 依次将其他的图片写入到GIF文件当中
                end
                pic_num = pic_num + 1;
                pause(0.1)
