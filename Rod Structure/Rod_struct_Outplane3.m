...
for mm=1%N_x+1  
    ...
    for nn=102:102%length(disp_y)
        ...
        if ismember(nn,1:90)
            Damp_1=0.01
            Damp_2=0.1
        elseif ismember(nn,90:94)
            Damp_1=0.005
            Damp_2=0.05
        elseif ismember(nn,95:101)
            Damp_1=0.001
            Damp_2=0.02
        elseif ismember(nn,102)
            Damp_1=0.0005
            Damp_2=0.01
...
%             Damp_2=0.025
        ...
        while (kk<=100||flag1>0.000001*R0_ring)&&flag2&&(kk<5/Damp||kk<3000)
            ...
            if kk<300
                flag2=1;
            elseif (max(U_M2_flag(kk-100:kk))-min(U_M2_flag(kk-100:kk)))<1e7;%energy variation
                flag2=0;
            end
            ...
            if mod(kk,100)==2
                ...
                subplot(2,2,2)
                if kk>100
                    plot(U_M2_flag(kk-100:kk)-U_M2_flag(kk-100))
                end
                pause(0.01)
            end
        end
        save('Rod_BiSym_Outplane3.mat',"Q_sec_M2","U_M2")
...
%%
load('Rod_BiSym_Outplane3.mat',"Q_sec_M2")
pic_num=1
...
%                 set(h, 'ZDir', 'reverse');
                ...
                [Amap, map] = rgb2ind(Im, 256); % 将RGB图片转化为索引图片
                if pic_num == 1
                    imwrite(Amap, map, 'Rod_BiSym_Outplane3.gif', 'gif', 'Loopcount', Inf, 'DelayTime', 0.1);
                    % 将第一张图片写入‘sin.gif’文件中，并且将GIF播放次数设置成无穷，即保存的GIF图会一直动下去
                else
                    imwrite(Amap, map,'Rod_BiSym_Outplane3.gif','gif','WriteMode','append','DelayTime',0.1);
                    % 依次将其他的图片写入到GIF文件当中
                end
                pic_num = pic_num + 1;
                pause(0.1)
