...
A=1e9;%the penalty parameter to constrain the frame vector Y Z as orthonormal
...
for nn=1:(size(Q_sec,2)-1)
    q_node=Curve_interp(Q_sec(:,nn),Q_sec(:,nn+1),L_e_0,N_node);
...
while (kk==0||flag1>0.000001*R0_ring||flag2>0.10)&&kk<2000
    kk=kk+1;
    [dFq_dq_sec,Fq_sec_M(:,mm),q_node,U_M(mm)] =Jocob_rod_sec(Q_sec,N_e_0,L_e_0,N_node,Par_E,A);
...
% load('Band_BiSym_Inplane.mat',"Q_sec_M1")
...
for mm=1:N_x+1
    ...
    while (kk<=100||flag1>0.000001*R0_ring||flag2>0.10)&&kk<2000
        kk=kk+1;
        [dFq_dq_sec,Fq_sec_M1(:,mm),q_node,U_M1(mm)] =Jocob_rod_sec(Q_sec,N_e_1,L_e_1,N_node,Par_E,A);
        F_tendon_1(mm)=Fq_sec_M1(1,mm);
        dFq_dq_sec_2=Proj_M'*dFq_dq_sec*Proj_M;
    ...
    if pic_num == 1
        imwrite(Amap, map, 'Rod_inplane.gif', 'gif', 'Loopcount', Inf, 'DelayTime', 0.1);
        % 将第一张图片写入‘sin.gif’文件中，并且将GIF播放次数设置成无穷，即保存的GIF图会一直动下去
    else
        imwrite(Amap, map,'Rod_inplane.gif','gif','WriteMode','append','DelayTime',0.1);
        % 依次将其他的图片写入到GIF文件当中
    end
    pic_num = pic_num + 1;
    pause(0.1)
...
subplot(2,1,2)
plot(disp_x,-F_tendon)
save('Rod_BiSym_Inplane.mat',"Q_sec_M1")

%% caculate bisymmetric configurations in the range inside the boundary
load('Rod_BiSym_Inplane.mat',"Q_sec_M1")
load('Rod_BiSym_Outplane4.mat')
A=1e9;
...
D_fix(4:12,1)=1;%fix the X ratation of Y Z axis
%end point
...
for mm=1%N_x+1  
    ...
    for nn=99:100%length(disp_y)
        ...
        if ismember(nn,1:5)
            Damp_1=0.01
            Damp_2=0.1
        elseif ismember(nn,6:33)
            Damp_1=0.005
            Damp_2=0.05
        elseif ismember(nn,34:37)
            Damp_1=0.002
            Damp_2=0.02
        elseif ismember(nn,38:90)
            Damp_1=0.001
            Damp_2=0.02
        elseif ismember(nn,91:95)
            Damp_1=0.0005
            Damp_2=0.02
        elseif ismember(nn,96:98)
            Damp_1=0.0001
            Damp_2=0.005
        elseif ismember(nn,99:102)
            Damp_1=0.0001
            Damp_2=0.002
...
%             Damp_2=0.025
        ...
        if ismember(nn,1:30)
            Q_sec(3,1)=Q_sec(3,1)+0.1*(15-0.5*nn);% add the z axis distorion
        end
        ...
        flag2=1;
        Damp=Damp_1
        while (kk<=100||flag1>0.000001*R0_ring)&&flag2&&(kk<5/Damp||kk<5000)
            kk=kk+1;
            Damp=0.995^(kk)*Damp_1+(1-0.995^(kk))*Damp_2;
            [dFq_dq_sec,Fq_sec_M2(:,nn),q_node,U_M2{mm}(nn)] =Jocob_rod_sec(Q_sec,N_e_1,L_e_1,N_node,Par_E,A);
            U_M2_flag(kk)=U_M2{mm}(nn);            
            ...
            if kk<=(2/Damp_2)
                flag2=1;
            elseif (max(U_M2_flag((kk-2/Damp_2):kk))-min(U_M2_flag((kk-2/Damp_2):kk)))<5e7;%energy variation
                flag2=0;
            end
            ...
            if mod(kk,100)==2
                ...
                subplot(2,2,2)
                if kk>(2/Damp_2)
                    plot(U_M2_flag((kk-2/Damp_2):kk)-U_M2_flag(kk-2/Damp_2))
                end
                pause(0.01)
            end
        end
        save('Rod_BiSym_Outplane4.mat',"Q_sec_M2","U_M2","F_tendon_2")
...
%         hold on
        ...
        subplot(2,2,4)
        plot(Y_2(1)-Y_2,abs(F_tendon_2{mm}(1:2,1:nn)))
        box on
        grid on
...
U_M=U_M2{mm}(1:nn);
figure(5);
plotyy(Y_2(1)-Y_2,U_M,Y_2(1)-Y_2,Z_1)
box on
grid on
xlabel('X')
ylabel('Y')
figure(6)
plot(Y_2(1)-Y_2,F_tendon_2{mm}(1,1:nn))

% save('Eye_BiSym_Outplane1.mat',"Q_sec_M2",'-append')
%%
load('Rod_BiSym_Outplane4.mat',"Q_sec_M2")
pic_num=1
for nn=1:length(disp_y)
    ...
    clf
    hold on
    Rod_ploting1(Q_sec,Duplication_M,L_co,C_coord)
    P_wheel=Q_sec(1:3,N_e_0+1);
    RM_wheel=reshape(Q_sec(4:12,N_e_0+1),3,3);
    Wheel_ploting(P_wheel,RM_wheel,R_wheel,N_spoke,Duplication_M)
    hold off
    ...
    [Amap, map] = rgb2ind(Im, 256); % 将RGB图片转化为索引图片
    if pic_num == 1
        imwrite(Amap, map, 'Rod_BiSym_Outplane4.gif', 'gif', 'Loopcount', Inf, 'DelayTime', 0.1);
        % 将第一张图片写入‘sin.gif’文件中，并且将GIF播放次数设置成无穷，即保存的GIF图会一直动下去
    else
        imwrite(Amap, map,'Rod_BiSym_Outplane4.gif','gif','WriteMode','append','DelayTime',0.1);
        % 依次将其他的图片写入到GIF文件当中
    end
    pic_num = pic_num + 1;
    pause(0.1)
