...
Proj_M(12*N_e_1+(1:3),12*N_e_1+(10:12))=Con_M_2;
%  couple local frame
...
Proj_M(12*(N_e_1+1)+(1:3),(10:12))=Con_M_2;
%  couple local frame 
...
Proj_M(12*(2*N_e_1+1)+(1:3),12*N_e_1+(10:12))=Con_M_2;
%  couple local frame 
...
for mm=1:N_x+1
    ...
    while (kk<=100||flag1>0.000001*R0_ring||flag2>0.10)&&kk<2000
        ...
        dFq_dq_sec_rd2=dFq_dq_sec_rd1;
        Fq_sec_M1_rd2(:,mm)= Fq_sec_M1_rd1(:,mm);
        q_node_rd2=q_node_rd1;
        U_M2_rd2(mm)=U_M1_rd1(mm);
...
for mm=1%N_x+1  
    ...
    for nn=98:102%length(disp_y)
        ...
        if ismember(nn,1:90)
            Damp_1=0.01
            Damp_2=0.1
        elseif ismember(nn,90:97)
            Damp_1=0.002
            Damp_2=0.1
        elseif ismember(nn,98:102)
            Damp_1=0.001
            Damp_2=0.01
...
%             Damp_2=0.025
        ...
        if ismember(nn,1:30)
            ...
            Q_sec(3,N_e_1+2)=Q_sec(3,N_e_1+2)+0.2*(15-0.5*nn);% add the z axis distorion
        else
            Q_sec(3,1)=Q_sec(3,1);
...
%             [dFq_dq_sec_rd2,Fq_sec_M1_rd2(:,mm),q_node_rd2,U_M1_rd2{mm}(nn)] =Jocob_rod_sec(Q_sec_rd2,N_e_1,L_e_1,N_node,Par_E,A);
            F_tendon(:,nn)=Fq_sec_M1_rd1(1:2,mm);
            dFq_dq_sec_rd2=dFq_dq_sec_rd1;
            Fq_sec_M1_rd2(:,mm)= Fq_sec_M1_rd1(:,mm);
            q_node_rd2=q_node_rd1;
            U_M1_rd2{mm}(nn)=U_M1_rd1{mm}(nn);
...
%             end
            ...
            P_wheel(:,1:2)=P_wheel(:,3)*[1 1];
            P_wheel(:,4:5)=P_wheel(:,6)*[1 1];
            P_wheel(:,7:8)=P_wheel(:,9)*[1 1];
        ...
        save('Rod_BiSym_Outplane4.mat',"Q_sec_M2","U_M1_rd1",'U_M1_rd2')
...
%                 set(h, 'ZDir', 'reverse');
                ...
                [Amap, map] = rgb2ind(Im, 256); % 将RGB图片转化为索引图片
                if pic_num == 1
                    imwrite(Amap, map, 'Band_BiSym_Outplane4.gif', 'gif', 'Loopcount', Inf, 'DelayTime', 0.1);
                    % 将第一张图片写入‘sin.gif’文件中，并且将GIF播放次数设置成无穷，即保存的GIF图会一直动下去
                else
                    imwrite(Amap, map,'Band_BiSym_Outplane4.gif','gif','WriteMode','append','DelayTime',0.1);
                    % 依次将其他的图片写入到GIF文件当中
                end
                pic_num = pic_num + 1;
                pause(0.1)
