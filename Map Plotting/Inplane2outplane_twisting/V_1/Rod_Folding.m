...
A=0.5e8;%can be to high or too low, depending on overall elatic energy level.
% L_e=L_e*10/9
%
...
% D_fix([10:12],1)=1;%fix the rotation
%
% r2
% position:
% Free: Y
...
D_fix(2,end)=1;%Y disp in control
...
% displacement x
disp_x=linspace(0,50,51);
% Angle_y=linspace(0,90,19)*pi/180;
%define the start points and frames
U=0*disp_x;
for mm=1:51
    mm
    Q_sec(1,1)=50-disp_x(mm);
    Q_sec(2,end)=50-disp_x(mm);
    if ismember(mm,[1 2])
...
for nn=1:size(disp_x,2)
    nn
    if (18+nn*2)>90
        Beta_y=[0:2:90];
    else
        Beta_y=linspace(0,18+nn*2,46);
    end
    ...
    for mm=1:size(Beta_y,2)
        ...
        if mm==size(Beta_y,2)&&mod(nn,5)==1
            figure(1)
            C_coord=[.0 .45 .74; .74 .45 .0];
            % cla
            for ii=1:N_e
    ...
    save('Rod_Folding.m','disp_x','disp_x_M','TAngle_M',"U_M",'disp_x_min',"TAngle_min",'U_min','Q_sec_MM')
