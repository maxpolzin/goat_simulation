...
Par_E=[E*pi*R_rod^2, E*pi*R_rod^4/4 G*pi*R_rod^4/2]';% the axis 
mm=101
nn=1
...
for jj=1:length(T_angle)
    ...
    flag2=1;
    L_step=0.05;
    while (kk<=100||flag1>0.000001*R0_ring)&&flag2&&(kk<5/Damp||kk<500)
        kk=kk+1;
        Damp=0.995^(kk)*Damp_1+(1-0.995^(kk))*Damp_2;
    ...
    if jj<8||mod(jj,1)==0||jj==length(T_angle)
        ...
        figure(4);
        subplot(4,1,1)
        plotyy(1:jj,U_Cal_sec3{mm}(1:jj),1:jj,U_v_sec3{mm}(1,1:jj))        
        ylabel('U')
        subplot(4,1,2)
        plot(Y_2(1:jj))
        box on
