...
for mm=1:N_x+1
    ...
    if pic_num == 1
        imwrite(Amap, map, 'Band_inplane.gif', 'gif', 'Loopcount', Inf, 'DelayTime', 0.1);
        % 将第一张图片写入‘sin.gif’文件中，并且将GIF播放次数设置成无穷，即保存的GIF图会一直动下去
    else
        imwrite(Amap, map,'Band_inplane.gif','gif','WriteMode','append','DelayTime',0.1);
        % 依次将其他的图片写入到GIF文件当中
    end
    pic_num = pic_num + 1;
    pause(0.1)
...
%             Damp_2=0.05
        ...
        while (kk<=100||flag1>0.000001*R0_ring)&&flag2&&(kk<5/Damp||kk<3000)
            ...
            if kk<300
                flag2=1;
            elseif (max(U_M1(kk-100:kk))-min(U_M1(kk-100:kk)))<1e1;%energy variation
                flag2=0;
            end
        ...
        save('Band_BiSym_Outplane4.mat',"Q_sec_M2","U_M1_rd1",'U_M1_rd2')
...
%%
load('Band_BiSym_Outplane4.mat',"Q_sec_M2")
pic_num=1
for nn=1:length(disp_y)
