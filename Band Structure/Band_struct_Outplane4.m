...
%             Damp_2=0.05
        ...
        save('Band_Outplane4.mat',"Q_sec_M2","U_M1_rd1",'U_M1_rd2')
...
%%
load('Band_Outplane4.mat',"Q_sec_M2")
pic_num=1
for nn=1:length(disp_y)
...
%                 set(h, 'ZDir', 'reverse');
                ...
                [Amap, map] = rgb2ind(Im, 256); % 将RGB图片转化为索引图片
                if pic_num == 1
                    imwrite(Amap, map, 'Band_Outplane4.gif', 'gif', 'Loopcount', Inf, 'DelayTime', 0.1);
                    % 将第一张图片写入‘sin.gif’文件中，并且将GIF播放次数设置成无穷，即保存的GIF图会一直动下去
                else
                    imwrite(Amap, map,'Band_Outplane4.gif','gif','WriteMode','append','DelayTime',0.1);
                    % 依次将其他的图片写入到GIF文件当中
                end
                pic_num = pic_num + 1;
                pause(0.1)
